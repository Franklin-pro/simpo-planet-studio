...
export default function ManageArtist() {
  ...
  const filterByDate = (artist: Artist) => {
    const now = new Date();
  ...
  const filteredArtists = artists
    .filter((a: Artist) => a.name.toLowerCase().includes(searchQuery.toLowerCase()))
    .filter(filterByDate);
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                />
                <p className="text-xs text-gray-500 mt-1">Current: {editingArtist.imageUrl}</p>
              </div>
              <div>
